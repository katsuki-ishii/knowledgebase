...
---
# [[基礎 Part 27 v-for 配列|v-for]] でオブジェクトを扱う
...
## 4. [[基礎 Part 2 index.htmlとmain.js|index]] と [[基礎 Part 27 v-for 配列|key]] は省略できるか

### 結論

- [[基礎 Part 27 v-for 配列|v-for]] の引数としては省略できる
- だが、:[[基礎 Part 27 v-for 配列|key]] 属性にキー名を使う必要があるため「[[基礎 Part 27 v-for 配列|key]] を省略すると現実的に困る」
...
## 5. [[基礎 Part 2 index.htmlとmain.js|index]] を [[基礎 Part 27 v-for 配列|key]] に使うべきでない理由
...
- [[基礎 Part 27 v-for 配列|key]] は実質必須（:[[基礎 Part 27 v-for 配列|key]] に使うため）
- [[基礎 Part 2 index.htmlとmain.js|index]] はほぼ不要
- 安定した [[基礎 Part 27 v-for 配列|key]] を付けることが重要

---

## 7. 実際のアプリ開発での使用例

オブジェクトを [[基礎 Part 27 v-for 配列|v-for]] で回す場面は実務でも頻出する。以下に代表的な例を示す。
...
以上のように、オブジェクトをそのまま並べて画面に出したい場面では [[基礎 Part 27 v-for 配列|v-for]] が自然に使われることが多い。
