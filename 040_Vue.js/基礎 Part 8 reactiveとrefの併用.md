...
Vue 3の[[基礎 Part 20 リアクティブエフェクト|リアクティブ]]システムには、2つの主要な関数がある。

| 関数 | 対象 | 主な用途 | 備考 |
| --- | --- | --- | --- |
| `reactive()` | オブジェクト・配列 | 複数のプロパティを持つデータを[[基礎 Part 20 リアクティブエフェクト|リアクティブ]]化 | Proxyで包む |
| `ref()` | プリミティブ値 | 数値や文字列など単一の値を[[基礎 Part 20 リアクティブエフェクト|リアクティブ]]化 | `{ value: ... }` で包む |
...
このため、Vueはプリミティブ値を `{ value: ... }` というオブジェクトで包み、そのオブジェクトを`reactive`化することで[[基礎 Part 20 リアクティブエフェクト|リアクティブ]]性を実現している。

---

## 4. 混乱を避けるための指針

1. オブジェクトを[[基礎 Part 20 リアクティブエフェクト|リアクティブ]]にしたいときは `reactive()` を使う。
2. 単一の値を[[基礎 Part 20 リアクティブエフェクト|リアクティブ]]にしたいときは `ref()` を使う。
