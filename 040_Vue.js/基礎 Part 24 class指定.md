...
---
# Vue における [[基礎 Part 24 class指定|class]] 指定

## 1. HTML の [[基礎 Part 24 class指定|class]] をどう扱うかという前提
...
文字列で [[基礎 Part 24 class指定|class]] を手動生成すると、条件が入るたびに煩雑になり破綻しやすい。Vue の [[基礎 Part 24 class指定|class]] バインディングはその問題を回避するためにある。

## 2. [[基礎 Part 24 class指定|class]] バインディングの3つの主要な書き方

### 2.1 文字列指定（最も単純）

固定の [[基礎 Part 24 class指定|class]] を書く場合はこれで十分。
...
特定の [[基礎 Part 24 class指定|class]] を状態に応じて付けたり外したりする場合に最適。
...
### 2.3 配列指定（[[基礎 Part 24 class指定|class]] 名そのものを動的に組み立てる）

変数として [[基礎 Part 24 class指定|class]] 名を渡したい場合や、複数クラスをまとめたい場合に使う。
...
className の値を書き換えるとそのまま [[基礎 Part 24 class指定|class]] が切り替わる。
...
## 3. 条件分岐が複雑化する前に [[基礎 Part 19 computed|computed]] に逃す

template に長いバインディングを書くと読みにくくなるため、[[基礎 Part 19 computed|computed]] にまとめる戦略が重要。
...
Tailwind はクラス名が長く、数が増えやすい。そのため Vue の [[基礎 Part 24 class指定|class]] バインディングが非常に有効。
...
## 4.5 [[基礎 Part 24 class指定|class]] 名を動的にするときの配列と [[基礎 Part 19 computed|computed]] の使い分け

[[基礎 Part 24 class指定|class]] 名そのものを切り替えたい場合、Vue では配列と [[基礎 Part 19 computed|computed]] のどちらでも実現できる。ただし用途によって適切な選択がある。
...
### [[基礎 Part 19 computed|computed]] を選ぶべきケース
...
### 例: 状態によって複数 [[基礎 Part 24 class指定|class]] が変わる場合
...
## 5. クラス爆発を防ぐための UI [[基礎 Part 30 コンポーネント|コンポーネント]]化

Tailwind を大量に並べると可読性が崩壊する。一定以上複雑になったら[[基礎 Part 30 コンポーネント|コンポーネント]]化するのが正解。
...
内部で Tailwind の [[基礎 Part 24 class指定|class]] を集約すれば、template を常にシンプルに保てる。
