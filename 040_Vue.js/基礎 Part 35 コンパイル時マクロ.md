...
---
# Vueにおける[[基礎 Part 35 コンパイル時マクロ|コンパイル時マクロ]]まとめ

## 1. [[基礎 Part 35 コンパイル時マクロ|コンパイル時マクロ]]とは何か

[[基礎 Part 35 コンパイル時マクロ|コンパイル時マクロ]]とは、**プログラムを実行する前（[[コンパイルとビルド|ビルド]]時）に、コードを解析・書き換えするための特別な記法**である。
...
- .vue ファイルを解析する
- `<script setup>` を通常の[[基礎 Part 30 コンポーネント|コンポーネント]]定義に変換する
- [[基礎 Part 35 コンパイル時マクロ|コンパイル時マクロ]]を解釈する
...
## 4. 図解：[[基礎 Part 35 コンパイル時マクロ|コンパイル時マクロ]]の位置づけ
...
## 5. 具体例①：[[基礎 Part 36 defineProps()|defineProps]]
...
## 6. 具体例②：[[基礎 Part 35 コンパイル時マクロ|defineEmits]]
...
これも同様に、[[基礎 Part 15 イベントオブジェクト|イベント]]定義をコンパイラに伝えるための[[基礎 Part 35 コンパイル時マクロ|マクロ]]であり、実行時に関数として処理されるわけではない。

## 7. 実行時関数との違い

| 項目 | [[基礎 Part 35 コンパイル時マクロ|コンパイル時マクロ]] | 実行時関数 |
| --- | --- | --- |
...
Vue の[[基礎 Part 35 コンパイル時マクロ|コンパイル時マクロ]]とは、`<script setup>` 内で使用される、Vue コンパイラ専用の構文である。
