...
Vue.js アプリ全体の「[[基礎 Part 30 コンポーネント|ルートコンポーネント]]（root [[基礎 Part 30 コンポーネント|component]]）」であり、アプリの土台となる部分。`main.js` から読み込まれ、アプリ全体をブラウザの `#app` にマウントする。
...
| `<style scoped>` | [[基礎 Part 30 コンポーネント|コンポーネント]]固有のスタイルを適用する |
...
- [[基礎 Part 34 グローバルCSS|グローバル]]状態やテーマを監視する
...
### 1. [[基礎 Part 34 グローバルCSS|グローバル]] [[ULCOD CSS|CSS]] を直接書く

[[基礎 Part 3 App.vue|App.vue]] に `scoped` なしで [[ULCOD CSS|CSS]] を書くと、全[[基礎 Part 30 コンポーネント|コンポーネント]]に影響してしまう。
...
- [[基礎 Part 34 グローバルCSS|グローバル]] [[ULCOD CSS|CSS]] は `src/assets/main.css` にまとめる
- Tailwind クラスで完結させる
...
### 5. [[基礎 Part 34 グローバルCSS|グローバル]]な[[基礎 Part 21 副作用と純粋関数|副作用]]をここで実行する

ログイン検証や[[基礎 Part 15 イベントオブジェクト|イベント]]リスナー登録などの[[基礎 Part 21 副作用と純粋関数|副作用]]を [[基礎 Part 3 App.vue|App.vue]] に書くと、他の部分に影響する。

**解決策：** `onMounted` などの[[基礎 Part 21 副作用と純粋関数|副作用]]は専用 composable（例: `useAuth.js`）にまとめる。

### 6. [[基礎 Part 34 グローバルCSS|グローバル]]スタイルをハードコードする
...
| スコープ付き [[ULCOD CSS|CSS]] (`scoped`) | 他[[基礎 Part 30 コンポーネント|コンポーネント]]への影響を防ぐ |
| Tailwind の利用 | [[基礎 Part 34 グローバルCSS|グローバル]] [[ULCOD CSS|CSS]] を最小限に抑え、統一デザインを保つ |
| [[基礎 Part 3 App.vue|App.vue]] は構造に専念 | ロジックや[[基礎 Part 21 副作用と純粋関数|副作用]]を持たない |
| 共通[[基礎 Part 30 コンポーネント|コンポーネント]]を明確に分離 | 再利用性を高める |
...
- [[基礎 Part 3 App.vue|App.vue]] に[[基礎 Part 34 グローバルCSS|グローバル]] [[ULCOD CSS|CSS]] やビジネスロジックを入れるのは責務分離違反
- 共通 UI [[基礎 Part 30 コンポーネント|コンポーネント]]は [[基礎 Part 3 App.vue|App.vue]] に、動的ロジックは composable に分離する
