...
aliases: [IPsecとVPN, IPsec, VPN, ipsec, vpn]
---
# [[IPsecとVPN|IPsec]]の概要

- **IP Security（[[IPsecとVPN|IPsec]]）**: ネットワーク層で通信を暗号化・認証・改ざん検知するプロトコル群。
...
- **トンネルモード**: IPパケット全体を暗号化し、新しいIPヘッダを付与。拠点間[[IPsecとVPN|VPN]]向け。
...
- **IKE (Internet [[基礎 Part 26 v-for 配列|Key]] Exchange)**: 鍵交換プロトコル。フェーズ1で安全チャネル、フェーズ2でデータ用鍵を確立。
...
3. **ESPヘッダ**（SPI: Security Parameters [[基礎 Part 2 index.htmlとmain.js|Index]]、シーケンス番号）を追加。
...
- **[[基礎 Part 9ブロックとブロックスコープ|ブロック]]暗号**: 一定サイズ（128ビット）のデータ単位ごとに暗号化。
- 鍵長: 128 / 192 / 256ビット。
- モード例:
    - **CBC（Cipher [[基礎 Part 9ブロックとブロックスコープ|Block]] Chaining）**: 前の暗号[[基礎 Part 9ブロックとブロックスコープ|ブロック]]と混ぜて次を暗号化。
    - **GCM（Galois/Counter Mode）**: 高速で並列処理可能、改ざん検知機能も付属。

## [[IPsecとVPN|VPN]]で遅くなる主な原因

7. 暗号化処理のCPU負荷（暗号化アクセラレータ未使用）。
8. 重い暗号方式（AES-256-CBCなど）。
9. MTU超過による断片化・再送。
10. 経路の迂回（[[IPsecとVPN|VPN]]ゲートウェイ位置による）。
11. 回線やQoS制限。
- **MTU（Maximum Transmission Unit）**: 一度に送れる最大データサイズ（例: 大きすぎる荷物は[[基礎 Part 13 分割代入|分割]]して送る必要がある）。
- **QoS（Quality of Service）**: 通信の優先度や帯域を制御する仕組み（例: 緊急車両を優先的に通す道路）。

## [[IPsecとVPN|VPN]]ルータの通信経路について

- **[[IPsecとVPN|VPN]]ルータ**は、[[IPsecとVPN|VPN]]接続の有無にかかわらず、通常その拠点やネットワークの出口として使われる。
- **[[IPsecとVPN|VPN]]接続時**: [[IPsecとVPN|VPN]]ルータは暗号化（[[IPsecとVPN|IPsec]]など）を行い、データを[[IPsecとVPN|VPN]]トンネルに送る。
    - 例: 東京の拠点ルータが大阪の拠点ルータに[[IPsecとVPN|IPsec]]トンネルでデータを送る。
- **[[IPsecとVPN|VPN]]未接続時**: 同じルータを通るが、暗号化せずに通常のインターネットルーティングとして通信を処理。
    - 例: 社内PCからインターネットのWebサイトへアクセスするときは暗号化されず、そのまま外部に転送される。
- 実務では、ルータが[[IPsecとVPN|VPN]]用と通常インターネット用の両方の経路を持ち、宛先によって使い分ける設定（ルーティング）がされていることが多い。