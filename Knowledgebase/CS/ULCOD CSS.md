...
---
# RESTの6つのスタイル（ULCOD CSS）まとめ

REST（Representational State Transfer）は、HTTP APIの書き方ではなく、**分散システムを長期的にスケールさせるための設計思想**である。

本質は「サーバが管理するリソースの状態を、表現（Representation）を通じて遷移させる」ことにある。

以下では、これまでの理解を踏まえ、RESTを構成する6つのスタイルを**思想 → 意味 → 具体例**の順で整理する。
...
RESTは、クライアントとサーバの間に存在する**中間層が通信の意味を理解できる**ことを重視する。
...
Uniform Interfaceは、RESTにおける最重要スタイルであり、他のスタイルを成立させる前提条件である。
...
これはHTTPを単なるトンネルとして使っており、RESTの利点を失う。
...
Client / Server 分離は、RESTが「単一アプリケーション」ではなく、**長期間・多様な利用者に耐える基盤**であるための前提条件である。
...
Cacheは、RESTが**ネットワーク越しで遅く不安定な通信**を前提としていることから生まれたスタイルである。
...
Stateless Serverは、RESTが**サーバ増減を前提とした設計**であることを示す。

サーバがクライアントの文脈を保持すると、分散配置が困難になる。

RESTでは、状態はすべてリソースに集約され、サーバは単なる処理装置となる。
...
Code on Demandは、RESTの中で唯一、**クライアントの単純性を犠牲にして柔軟性を得るスタイル**である。
