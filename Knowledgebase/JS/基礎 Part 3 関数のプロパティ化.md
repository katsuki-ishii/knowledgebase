...
aliases: [関数のプロパティ化, 関数プロパティ化, 関数のプロパティ, ファーストクラスオブジェクト]
---
## 📝 問題文（日本語訳）
...
- `if (条件)` の後ろに[[基礎 Part 5セミコロン・パース|セミコロン]]を付けない。付けると意図しない動作になる。

### ◉ よくあるミス

- `if (条件);` ← [[基礎 Part 5セミコロン・パース|セミコロン]]を付けてしまう
...
- **[[基礎 Part 3 関数のプロパティ化|ファーストクラスオブジェクト]]**とは？
    - 関数も「値」として変数に入れたり、引数や戻り値にできたり、オブジェクトのプロパティにできる言語仕様。
    - JavaScriptは関数が[[基礎 Part 3 関数のプロパティ化|ファーストクラスオブジェクト]]。
- **セカンドクラス？**
...
### ◉ この書き方は「[[基礎 Part 10 関数の定義方法3つ|アロー関数]]式」の省略形
...
- `func` は関数を[[基礎 Part 8 宣言・代入・再代入|代入]]するための変数名（関数名）
- `= () =>` は「無名関数（[[基礎 Part 10 関数の定義方法3つ|アロー関数]]）」を[[基礎 Part 8 宣言・代入・再代入|代入]]している
    - `()` は引数なし
    - `=>` は「…を返す」という意味
...
- [[基礎 Part 5セミコロン・パース|セミコロン]]の位置に注意
- JavaScriptのテストライブラリもこの仕組みがベース
- `func = () => ...` は[[基礎 Part 10 関数の定義方法3つ|アロー関数]]で「即席の無名関数」を作る書き方
