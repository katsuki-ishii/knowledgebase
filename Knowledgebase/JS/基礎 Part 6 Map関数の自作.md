...
- `const`で配列を宣言し、中身だけ変更するスタイルが推奨。

---

## 3. letとconstの使い分け

- **再代入しない変数**（配列の入っている箱そのものを変えない）：`const`
- **再代入する変数**（ループカウンタなど）：`let`
...
- `const`で配列やオブジェクトを宣言しても中身は自由に変更できる
- mapの再実装は「コアな配列・関数の理解」に役立つ良問
