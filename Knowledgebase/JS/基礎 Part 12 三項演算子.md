...
---
三項演算子（ternary operator）は、if 文を簡潔に書くための構文である。
...
上記の2つは同じ意味になる。三項演算子を使うことで、1行で簡潔に書ける。
...
三項演算子は「式（expression）」であり、値を返す。

一方、if 文は「文（statement）」であり、値を返さない。

このため、三項演算子は変数への代入や関数の戻り値に直接使える。
