...
`reactive`関数は、Vue.jsのComposition APIで提供されるリアクティブシステムの一部であり、**オブジェクトや配列をリアクティブに変換するための関数**である。リアクティブとは、データが変化したときに自動的にビュー（画面）へ反映される性質を指す。

Vue.jsでは、開発者が手動でDOMを更新するのではなく、状態（データ）を更新すればVueが自動的に画面を再描画する。このリアクティブな仕組みを作るために、`reactive`はオブジェクトを**Proxy（プロキシ）**でラップし、プロパティの変更を検知するようになっている。

---

## ref関数との違い

`ref`と`reactive`はどちらもリアクティブな状態を作るための関数だが、用途と動作に違いがある。

| 項目 | ref() | reactive() |
| --- | --- | --- |
| 対象 | プリミティブ値（数値・文字列など） | オブジェクト・配列など複合データ |
| リアクティブ化の範囲 | 1つの値のみ | オブジェクト全体（ネストも含む） |
...
1. **オブジェクト全体をリアクティブ化する**
オブジェクトや配列を渡すと、その中のすべてのプロパティが監視対象になる。
2. **ネストしたオブジェクトも自動でリアクティブになる**
内部に入れ子構造を持つ場合でも、すべて自動でリアクティブ化される。
3. `**.value**`** が不要**
...
5. 生成されたリアクティブオブジェクトを`ref`の`.value`に格納する。
...
この仕組みにより、`ref`を使えばプリミティブ値でもオブジェクトでも同じような書き方でリアクティブ変数を扱える。Vueが内部で適切に処理してくれるため、開発者は型を意識せずに`ref`を使うことができる。
...
8. `reactive`で作ったオブジェクトを分割代入すると、リアクティブ性が失われる。その場合は`toRefs()`を使用する。
```javascript
...
- `reactive`ではネストされたオブジェクトも自動的にリアクティブ化される。
