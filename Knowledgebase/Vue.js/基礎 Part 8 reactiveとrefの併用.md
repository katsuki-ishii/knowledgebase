...
Vue 3のリアクティブシステムには、2つの主要な関数がある。

| 関数 | 対象 | 主な用途 | 備考 |
| --- | --- | --- | --- |
| `reactive()` | オブジェクト・配列 | 複数のプロパティを持つデータをリアクティブ化 | Proxyで包む |
| `ref()` | プリミティブ値 | 数値や文字列など単一の値をリアクティブ化 | `{ value: ... }` で包む |
...
このため、Vueはプリミティブ値を `{ value: ... }` というオブジェクトで包み、そのオブジェクトを`reactive`化することでリアクティブ性を実現している。

---

## 4. 混乱を避けるための指針

1. オブジェクトをリアクティブにしたいときは `reactive()` を使う。
2. 単一の値をリアクティブにしたいときは `ref()` を使う。
