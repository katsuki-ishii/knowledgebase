...
- ロジックはすべて [[基礎 Part 15 Vueコード規約|composables]] に集約する。
- [[基礎 Part 29 コンポーネント|コンポーネント]]は UI と[[基礎 Part 14 イベントオブジェクト|イベント]]配線に特化させる。
- [[基礎 Part 10 setup構文|setup]] は長くなりすぎないようにし、肥大化した部分は [[基礎 Part 15 Vueコード規約|composables]] へ移動する。
...
- ページ固有[[基礎 Part 29 コンポーネント|コンポーネント]]が拡張されても、views 内で階層化できるので **スケールしやすい**。
...
- [[基礎 Part 15 Vueコード規約|composables]] の返り値は [[基礎 Part 4 ref関数|ref]] を基本とし、必要最小限だけ [[基礎 Part 5 reactive関数|reactive]] を返す。
- API の生データは [[基礎 Part 4 ref関数|ref]] に入れて、加工は [[基礎 Part 18 computed|computed]] で行う。

---

## 3. [[基礎 Part 15 Vueコード規約|composables]] の設計方針

- [[基礎 Part 15 Vueコード規約|composables]] は必ず関数として定義し、必要な値・関数のみ返す。
...
- 返り値の整形以上のロジックは [[基礎 Part 15 Vueコード規約|composables]] へ寄せる。
...
## 5. [[基礎 Part 15 Vueコード規約|Pinia]] ストアの方針

- 画面を跨いで必要な state を保持することを基本とする。
- ただし **同一画面内の複数[[基礎 Part 29 コンポーネント|コンポーネント]]間で状態共有が必要で、[[基礎 Part 35 defineProps()|props]] 受け渡しが複雑になる場合は [[基礎 Part 15 Vueコード規約|Pinia]] を使用してよい**。
- [[基礎 Part 15 Vueコード規約|composables]] はローカルな機能単位の状態保持に使う。
- store へ入れる state は「共有したい理由」が明確であることを条件とする。

### [[基礎 Part 15 Vueコード規約|Pinia]] を使うべきケース

- 画面を跨いで使うユーザー情報や設定値。
- 同一画面内で複数[[基礎 Part 29 コンポーネント|コンポーネント]]間にまたがる状態を共有する必要があり、[[基礎 Part 35 defineProps()|props]] バケツリレーが煩雑になる場合。
- 複雑な UI 状態（タブ、フィルター、選択状態など）を複数[[基礎 Part 29 コンポーネント|コンポーネント]]で扱う場合。

### [[基礎 Part 15 Vueコード規約|Pinia]] を使わない方がよいケース

- 単一[[基礎 Part 29 コンポーネント|コンポーネント]]内で完結する状態。
- 一時的な UI 状態（モーダル開閉、ローカルな検索キーワードなど）。
- [[基礎 Part 15 Vueコード規約|composables]] で簡単に共有できる程度の軽い状態。
...
## 6. [[基礎 Part 29 コンポーネント|コンポーネント]]の設計. [[基礎 Part 29 コンポーネント|コンポーネント]]の設計
...
- [[基礎 Part 29 コンポーネント|コンポーネント]]にはビジネスロジックを入れず [[基礎 Part 15 Vueコード規約|composables]] から取得する。

### [[基礎 Part 29 コンポーネント|コンポーネント]]例
...
3. [[基礎 Part 18 computed|computed]]
4. methods（関数）
5. [[基礎 Part 22 watch|watch]]
6. lifecycle hooks
7. return

## 8. [[基礎 Part 22 watch|watch]] と [[基礎 Part 18 computed|computed]] の方針

- [[基礎 Part 18 computed|computed]] は値の導出専用とし、[[基礎 Part 20 副作用と純粋関数|副作用]]を入れない。
- [[基礎 Part 22 watch|watch]] は[[基礎 Part 20 副作用と純粋関数|副作用]]が必要な場合にのみ使う。
- 可能であれば[[基礎 Part 14 イベントオブジェクト|イベント]]伝播や明示的な関数呼び出しで代替する。

## 9. [[基礎 Part 35 defineProps()|props]] / emits

- [[基礎 Part 35 defineProps()|defineProps]] / [[基礎 Part 34 コンパイル時マクロ|defineEmits]] を用いる。
- [[基礎 Part 35 defineProps()|props]] のデフォルト値は必要に応じて親で明示する。
- emits は UI の動作を表す名前にする（save, close など）。

## 10. views の方針

- ページの読み込みや画面遷移に関する最小限の処理だけを書く。
- ロジックは [[基礎 Part 15 Vueコード規約|composables]] へ逃がす。
...
- ロジックが [[基礎 Part 15 Vueコード規約|composables]] へ適切に分離されているか。
- 命名が用途を的確に表しているか。
- services が API 呼び出し専用として保たれているか。
- views や components が肥大化していないか。
- stores に余計なロジックが入っていないか。

## 13. 肥大化判断基準（[[基礎 Part 29 コンポーネント|コンポーネント]]・[[基礎 Part 15 Vueコード規約|composables]]・stores）

### [[基礎 Part 29 コンポーネント|コンポーネント]]（views / components）

- [[基礎 Part 10 setup構文|setup]] が 150 行を超えた場合は[[基礎 Part 13 分割代入|分割]]を検討する。
- 関数が 10 個を超える場合は責務過多と判断する。
- template が 200 行を超える場合は部分[[基礎 Part 29 コンポーネント|コンポーネント]]化を検討する。
- [[基礎 Part 35 defineProps()|props]] が 8 個以上の場合は[[基礎 Part 29 コンポーネント|コンポーネント]][[基礎 Part 13 分割代入|分割]]を検討する。

### [[基礎 Part 15 Vueコード規約|composables]]

- ファイルが 200 行を超える場合はロジック[[基礎 Part 13 分割代入|分割]]を検討する。
- return する変数や関数が 11 個以上になった場合は責務を[[基礎 Part 13 分割代入|分割]]する。
- [[基礎 Part 22 watch|watch]] が 3 つ以上ある場合はロジックが集中しすぎている可能性がある。

### stores（[[基礎 Part 15 Vueコード規約|Pinia]]）

- state が 10 項目以上ある場合は store の責務過多と判断する。
- actions が 10 個以上ある場合はロジックを [[基礎 Part 15 Vueコード規約|composables]] に移すことを検討する。
- 単一画面でしか使わない state は [[基礎 Part 15 Vueコード規約|composables]] または [[基礎 Part 29 コンポーネント|component]] 側に移動する。

これらの基準をもとに、[[基礎 Part 29 コンポーネント|コンポーネント]]や [[基礎 Part 15 Vueコード規約|composables]]、store の責務が適切に分担されているかを確認することを推奨する。
