...
aliases: [v-if, vif, v if, v-else, v-else-if]
---
# Vue.js [[基礎 v-if|v-if Part 24]] 

## 1. [[基礎 v-if|v-if Part 24]] の基本

[[基礎 v-if|v-if Part 24]] は「条件が true のときだけ HTML 要素を DOM に生成する」仕組み。false のときは要素そのものが DOM から消える。
...
必要なときだけ[[基礎 コンポーネント|コンポーネント Part 29]]を DOM に載せる。
...
複数の要素をまとめて [[基礎 v-if|v-if Part 24]] の対象にしたい場合、ラッパーとして **div** または **template** を使う。どちらを使うべきかは、UI の構造や DOM の意図によって変わる。
...
- **レイアウト上、囲む[[基礎 Part 9ブロックとブロックスコープ|ブロック]]が必要なとき**
- [[ULCOD CSS|CSS]] のスタイル適用のために親要素が欲しい場合
...
- [[ULCOD CSS|CSS]] の影響範囲を広げたくない場合に有効
...
- 見た目や意味的に“ひとまとまりの[[基礎 Part 9ブロックとブロックスコープ|ブロック]]”が必要 → **div**
- DOM を汚したくない、親要素がすでにある → **template**

この判断基準を持っておくと UI 設計の時に迷わなくなる。

複数の要素をまとめて [[基礎 v-if|v-if Part 24]] の対象にしたい場合、ラッパー要素で囲む。Vue では 1 つの [[基礎 v-if|v-if Part 24]] は 1 つのタグにしか付与できないため、グループ化が必要になる。
...
以上が実務での [[基礎 v-if|v-if Part 24]] の代表的な使い方になる。不足しているケースや、特定システムでの応用例が必要なら追加してまとめる。

## 関連
- [[基礎 Part 39 v-show Part 25]]
- [[基礎 v-for 配列]]
