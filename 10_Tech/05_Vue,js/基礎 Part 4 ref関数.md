...
---
# 1. [[基礎 Part 4 ref関数|ref()]] の基本

`ref()` は[[基礎 Part 19 リアクティブエフェクト|リアクティブ]]な値を作る関数で、内部的にオブジェクトを返す。
...
- プリミティブ値（数値・文字列・真偽値など）を[[基礎 Part 19 リアクティブエフェクト|リアクティブ]]化できる
- `.value` プロパティ経由でアクセスする
- `<template>` 内では `.value` を省略できる（テンプレートアンラップ）

---

## 2. [[基礎 Part 4 ref関数|ref()]] のリアクティビティ確認
...
プリミティブは「値渡し」であり、変更を検知できないため、そのままでは[[基礎 Part 19 リアクティブエフェクト|リアクティブ]]にできない。
...
Vue の[[基礎 Part 19 リアクティブエフェクト|リアクティブ]]システムは、このようにプロパティの読み書きを横取りして、変更を検知している。

`ref()` はプリミティブ値をオブジェクト（Proxyで監視可能）で包むことで、[[基礎 Part 19 リアクティブエフェクト|リアクティブ]]化を実現している。
...
| 概念 | 説明 |
| --- | --- |
| `ref()` | プリミティブ値をオブジェクトに包んで[[基礎 Part 19 リアクティブエフェクト|リアクティブ]]化する関数 |
| `.value` | [[基礎 Part 19 リアクティブエフェクト|リアクティブ]]な値の実体を持つプロパティ |
