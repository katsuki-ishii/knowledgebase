...
aliases: [ref関数, ref, ref(), Ref]
---
# 1. [[基礎 ref関数|ref() Part 4]] の基本

`ref()` は[[基礎 リアクティブエフェクト|リアクティブ Part 19]]な値を作る関数で、内部的にオブジェクトを返す。
...
- プリミティブ値（数値・文字列・真偽値など）を[[基礎 リアクティブエフェクト|リアクティブ Part 19]]化できる
- `.value` プロパティ経由でアクセスする
- `<template>` 内では `.value` を省略できる（テンプレートアンラップ）

---

## 2. [[基礎 ref関数|ref() Part 4]] のリアクティビティ確認
...
プリミティブは「値渡し」であり、変更を検知できないため、そのままでは[[基礎 リアクティブエフェクト|リアクティブ Part 19]]にできない。
...
Vue の[[基礎 リアクティブエフェクト|リアクティブ Part 19]]システムは、このようにプロパティの読み書きを横取りして、変更を検知している。

`ref()` はプリミティブ値をオブジェクト（Proxyで監視可能）で包むことで、[[基礎 リアクティブエフェクト|リアクティブ Part 19]]化を実現している。
...
| 概念 | 説明 |
| --- | --- |
| `ref()` | プリミティブ値をオブジェクトに包んで[[基礎 リアクティブエフェクト|リアクティブ Part 19]]化する関数 |
| `.value` | [[基礎 リアクティブエフェクト|リアクティブ Part 19]]な値の実体を持つプロパティ |
| プリミティブ型 | 値そのもの。変更を検知できない |
| オブジェクト型 | 参照を持つ。Proxyで監視可能 |
| テンプレートアンラップ | `<template>` 内で `.value` を省略できる仕組み |

## 関連
- [[基礎 Part 9 reactive関数 Part 5]]
