...
```
- この "[[基礎 Part 37 props バリデーション|type]]" のようなフィールドで通信の目的を区別し、アプリケーションが「どの処理か」を判定する。
- HTTPでのメソッドに相当するのが、[[Web Socket|WebSocket]]ではアプリケーション独自の[[基礎 Part 37 props バリデーション|type]]設計。
...
| --- | --- | --- |
| 通信の区別方法 | メソッド(GET/POST等) | データ内容で区別（[[基礎 Part 37 props バリデーション|type]]等で自作） |
